...
function getMetadata(entry: Entry) {
  return entry.type === "directory" ? entry.index?.metadata : entry.metadata;
}
...
function getLabel(entry: Entry): string {
  ...
  return (
    metadata?.sidebar?.label ?? metadata?.title ?? prettifyName(entry.name)
  );
}

function getOrder(entry: Entry): number {
  return getMetadata(entry)?.sidebar?.order ?? Number.MAX_SAFE_INTEGER;
}

function isHidden(entry: Entry): boolean {
  return getMetadata(entry)?.sidebar?.hidden === true;
}
...
function createFileItem(entry: FileEntry): SidebarItem | null {
  if (isHidden(entry)) {
    return null;
  }
...
function createDirectoryItem(
  ...
  const children = generateEntries(entry.children, options);
...
function generateEntry(
  ...
  if (!options.includeDrafts && isDraft(entry)) {
    return null;
  }
  ...
  return createDirectoryItem(entry, options);
}
...
function generateEntries(
  ...
  return sortEntries(entries)
    .map((entry) => generateEntry(entry, options))
...
export function generateSidebar(
  ...
  return {
    label: options.label,
    items: generateEntries(entries, options),
